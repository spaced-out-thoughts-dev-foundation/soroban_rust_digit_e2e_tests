...
def rust_to_dtr(input_string)
  c_string_pointer = MyRustLib.rust_string_to_dtr_string(input_string)
  raise 'Failed to convert Rust string to DTR string' if c_string_pointer.null?

  c_string = c_string_pointer.read_string
  # Free the C string when done
  MyRustLib.free_cstring(c_string_pointer)

  c_string
end

def run_cargo_test_in_dir(dir)
  Dir.chdir(dir) do
    _, _, status = Open3.capture3('cargo test')
